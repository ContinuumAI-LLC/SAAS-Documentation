...
| **502** | Bad Gateway | The upstream model provider returned an error. Retry or try a different model. |
...
<AccordionGroup>
  ...
  <Accordion title="UPSTREAM_ERROR" icon="circle-xmark">
    **HTTP status:** 502
    ...
    The upstream model provider returned an error or timed out.

    ```json
    {
      "error": {
        "message": "The upstream model provider returned an error. Please retry your request.",
        "code": "UPSTREAM_ERROR"
      },
      "status": 502
    }
        ...
        try {
          ...
          if (error instanceof OpenAI.RateLimitError) {
            ...
            const retryAfter = parseFloat(
              error.headers?.["retry-after"] ?? Math.pow(2, attempt)
            );
            ...
            console.log(`Connection error. Retrying in ${wait.toFixed(1)}s...`);
            await sleep(wait * 1000);

          } else if (
            error instanceof OpenAI.APIError &&
            [500, 502].includes(error.status)
          ) {
            // Server error: retry with backoff
            const wait = Math.min(Math.pow(2, attempt) + Math.random(), 60);
            console.log(`Server error (${error.status}). Retrying in ${wait.toFixed(1)}s...`);
            await sleep(wait * 1000);
